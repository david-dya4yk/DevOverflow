...
const RightSidebar = async () => {
  ...
  const [{success, data: hotQuestions, error}, {success: tagSuccess, data: tags, error: tagError}] = await Promise.all([getHotQuestion(),  getTopTags()])
  ...
  return (
    <section
      ...
      <div className=" ">
        ...
        <DataRenderer data={hotQuestions} success={success} error={error} empty={{
        ...
        <DataRenderer data={tags} success={tagSuccess} error={tagError} empty={{
          title: 'No tags found.',
          message: 'No tags have been created.'
        }} render={(tags) => (
          <div className='mt-7 flex flex-col gap-4'>
            {tags.map(({_id, name, questions}) =>
              <TagCard key={_id} _id={_id} questions={questions} name={name}
                       showCount compact />)}
          </div>
        )}/>
      </div>
